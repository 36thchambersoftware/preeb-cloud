...
export default async function handler(req, res) {
  ...
  const endpointParam = req.query.endpoint ?? req.query.path;
  const querySegments = normalizeSegments(
    Array.isArray(endpointParam)
      ? endpointParam
      : [endpointParam].filter(Boolean)
  );
  ...
  if (!targetPath) {
    res.status(400).json({
      error: 'Missing Koios path. Use /api/koios/pool_list?... or /api/koios?endpoint=pool_list&...',
    });
    return;
  }
  ...
  for (const [key, value] of Object.entries(req.query)) {
    if (key === 'path' || key === 'endpoint' || key === '...path' || key === '[...path]') continue;
